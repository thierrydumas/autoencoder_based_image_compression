...
def area_under_piecewise_linear_functions(parameters, nb_points_per_interval, nb_intervals_per_side):
    ...
    sum_slice_parameters_first_last_points = tf.reshape(tf.slice(parameters, [0, 0], [nb_maps, 1]) + tf.slice(parameters, [0, nb_points - 1], [nb_maps, 1]),
                                                        [-1])
    slice_parameters_all_excluding_first_last_points = tf.slice(parameters, [0, 1], [nb_maps, nb_points - 2])
    
    # For details on the condition below, see
    # <https://www.tensorflow.org/api_guides/python/upgrade>.
    if tf.__version__.startswith('0'):
        sum_slice_parameters_all_excluding_first_last_points = tf.reduce_sum(slice_parameters_all_excluding_first_last_points, 1)
    else:
        sum_slice_parameters_all_excluding_first_last_points = tf.reduce_sum(slice_parameters_all_excluding_first_last_points, axis=1)
    return 0.5*(sum_slice_parameters_first_last_points + 2.*sum_slice_parameters_all_excluding_first_last_points)/nb_points_per_interval
...
def differential_entropy(approximate_prob):
    ...
    """
    opposite_log2_approximate_prob = -tf.log(approximate_prob)/tf.log(2.)
    if tf.__version__.startswith('0'):
        return tf.reduce_mean(opposite_log2_approximate_prob, 1)
    else:
        return tf.reduce_mean(opposite_log2_approximate_prob, axis=1)
...
def expand_parameters(parameters, low_projection, nb_points_per_interval, nb_added_per_side):
    ...
    with tf.control_dependencies([tf.assert_positive(nb_added_per_side)]):
        ...
        # In `tf.concat`, the positional arguments changed
        # from the Tensorflow version 0.x to 1.x.
        if tf.__version__.startswith('0'):
            return tf.concat(1, [piece, parameters, piece])
        else:
...
def loss_density_approximation(approximate_prob, parameters, nb_points_per_interval):
    ...
    """
    parameters_squared = parameters**2
    if tf.__version__.startswith('0'):
        mean_approximate_prob = tf.reduce_mean(approximate_prob, 1)
        sum_parameters_squared = tf.reduce_sum(parameters_squared, 1)
    else:
        mean_approximate_prob = tf.reduce_mean(approximate_prob, axis=1)
        sum_parameters_squared = tf.reduce_sum(parameters_squared, axis=1)  
    return tf.reduce_sum(-2.*mean_approximate_prob + sum_parameters_squared/nb_points_per_interval)
...
def reconstruction_error(visible_units, reconstruction):
    ...
    """
    difference_squared = (visible_units - reconstruction)**2
    if tf.__version__.startswith('0'):
        sum_difference_squared = tf.reduce_sum(difference_squared, [1, 2, 3])
    else:
        sum_difference_squared = tf.reduce_sum(difference_squared, axis=[1, 2, 3])
    return tf.reduce_mean(sum_difference_squared)
